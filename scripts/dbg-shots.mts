...
m.restart(180);
let headers = 0, volleys = 0, shots = 0, saves = 0, corners = 0, penalties = 0, freekicks = 0, throwins = 0, goalkicks = 0;
for (let t = 0; t < 180 * 30 && m.phase !== 'ended'; t++) {
  ...
  for (const e of m.step()) {
    ...
    if ((e as any).tech === 'header') headers++;
    if ((e as any).tech === 'volley') volleys++;
...
console.log(JSON.stringify({ score: m.score, headers, volleys, shots, saves, corners, penalties, freekicks, throwins, goalkicks }));
process.exit(0);
